import { Container, Flex, Text, theme } from 'ooni-components'
  ...
  return (
    <>
      ...
      >
        ...
        <ClipboardIcon>
          {clipboard.copied && (
            <FaClipboard size={10} color={theme.colors.green7} />
          )}
        </ClipboardIcon>
      </StyledNameCell>
    </>
...
const ConnectionStatusCell = ({ cell: { value } }) => {
  ...
  if (value === false) {
    statusIcon = (
      <Text fontWeight="bold" fontSize={1} color={theme.colors.gray7}>
        N/A
      </Text>
    )
  } else {

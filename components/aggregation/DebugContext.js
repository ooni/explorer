...
export const DebugProvider = ({ children }) => {
  ...
  const doneTableReshaping = useCallback((t0, t1) => {
    if (t0 !== tableReshapeTimeRef.current.pre && t1 !== tableReshapeTimeRef.current.post) {
  ...
  const doneChartReshaping = useCallback((t0, t1) => {
    if (t0 !== chartsReshapeTimeRef.current.pre && t1 !== chartsReshapeTimeRef.current.post) {
  ...
  return (
    <DebugContext.Provider value={{ query, apiResponse, tableReshapeTimeRef, chartsReshapeTimeRef, renderTimeRef, scrollStartRef, doneTableReshaping, doneChartReshaping, doneRendering, others, debugQuery, debugApiResponse, addToDebug, startedScrolling}}>
      {children}

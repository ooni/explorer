...
const FindingDisplay = ({ incident }) => {
  ...
  const listOfNetworks = incident?.ASNs?.map((as) => (
    <NLink key={as} href={`/as/AS${as}`}>{`AS${as}`}</NLink>
  ...
  return (
    <>
      <Heading
        ...
        mb={4}
        pb={3}
        sx={{ borderBottom: '1px solid', borderColor: 'gray3' }}
      >
        {incident?.title}
      </Heading>
      {!!incident?.CCs?.length && (
      ...
      <Box fontSize={18} lineHeight="1.5">
        {incident?.text && (

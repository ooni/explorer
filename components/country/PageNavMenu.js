...
const PageNavItem = ({ link, children }) => (
  <div className="mx-4 my-1" mx={3} my={1}>
    <a className="p-2 text-base text-blue-600" href={link}>
      {children}
    </a>
  </div>
)
  ...
  return (
    <>
      ...
      <HideInLargeScreens large xlarge>
        <MdExpandLess
          className={`cursor-pointer bg-gray-400 rounded-[50%] transition-transform ease-linear ${isOpen ? 'rotate-0' : 'rotate-180'}`}
          size={36}
          open={isOpen}

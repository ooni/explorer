import { Flex, Link, Text } from 'ooni-components'
          ...
          { id: 'SocialButtons.CTA' },
          {
            ...
            'twitter-link': (string) => (
              <Link
                color="blue7"
                target="_blank"
                href={`https://twitter.com/intent/tweet?text=${intl.formatMessage(
                  { id: 'SocialButtons.Text' },
                )}&url=https://explorer.ooni.org/${url}`}
              >
                {string}
              </Link>
            ),

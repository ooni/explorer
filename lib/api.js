...
export const getReports = async (filter) => {
  const results = await Axios.get('https://ooni.org/pageindex.json')
  return getFilteredReports(filter, results.data)
}
...
export const getThematicData = async (theme) => {
  try {
    ...
    const selectedCountries = findings.length
      ? findings.flatMap((f) => f.CCs).slice(0, 3)
      : ['CN', 'IR', 'RU']

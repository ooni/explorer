...
export const getBaseUrl = (backend) => {
  const isClient = typeof window !== 'undefined'
  const useProxy = isClient && process.env.NEXT_PUBLIC_PROXY_API

  if (useProxy) {
    return backend === 'ooni' ? '/proxy/ooni' : '/proxy/feedback'
  }

  return backend === 'ooni'
    ? process.env.NEXT_PUBLIC_OONI_API
    : process.env.NEXT_PUBLIC_USER_FEEDBACK_API
}
...
export const toApiPath = (url) => {
  ...
  return url.replace(/^\/proxy\/ooni/, '').replace(/^\/proxy\/feedback/, '')
}
...
export const apiFetch = async (path, options = {}) => {
  ...
  const url =
    backend === null
      ...
      : buildUrl(path, params, getBaseUrl(backend))
...
const postAPI = async (endpoint, params) => {
  return getAPI(endpoint, null, { method: 'POST', data: params })
}
